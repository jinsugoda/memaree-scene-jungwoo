...
const GroupUserListSelected = (prop: UserlistProps) => {
    ...
    const removeUser = (user: User) => {
        const newUsers = prop?.selectedUsers?.filter((u) => u?._id !== user?._id);
        prop?.setSelectedUsers(newUsers);
        const newUnselectedUsers = prop?.unselectedUsers.concat(user);
        prop?.setUnselectedUsers(newUnselectedUsers);
    };

    return (
        <FlatList
            data={prop?.selectedUsers}
            style={UserListStyles.container}
            renderItem={(data) => (
                <TouchableOpacity
                    key={`T${data?.item?._id}`}
                    onPress={() => removeUser(data?.item)}
                >
                    <ListItem
                        containerStyle={{
                            paddingVertical: 6,
...
export default GroupUserListSelected;
